...
const getDateInTimezone = (day: string, timezone: Timezone) =>
  getMomentWithTimezone(day, timezone.toString()).toDate();
...
describe("monthToWeeks", () => {
  describe("March 2010 (summer time change forward)", () => {
    ...
    it("should calculate for UTC (no DST)", () => {
      ...
      assertEqualCalendarMatrix(
        monthToWeeks(firstMarch2010, utc, getLocale()),
        utcMarch2010,
      );
    });
    ...
    it("should calculate for Warsaw (DST)", () => {
      ...
      assertEqualCalendarMatrix(
        monthToWeeks(firstMarch2010, warsawTZ, getLocale()),
        warsawMarch2010,
      );
    });
  });
  ...
  describe("October 2019 (summer time change backward)", () => {
    ...
    it("should calculate for UTC october (no DST)", () => {
      ...
      assertEqualCalendarMatrix(
        monthToWeeks(firstOctober2019, utc, getLocale()),
        utcOctober2019,
      );
    });
    ...
    it("should calculate for Warsaw october (DST)", () => {
      ...
      const warsawOctober2019 = convertDatesToTimezone(
        october2019Weeks,
        warsawTZ,
      );
      assertEqualCalendarMatrix(
        monthToWeeks(firstOctober2019, warsawTZ, getLocale()),
        warsawOctober2019,
      );
    });
  });
...
describe("calendarDays", () => {
  it("should calculate calendar for March UTC", () => {
    ...
    assertEqualCalendarMatrix(
      calendarDays(firstMarch2010, utc, getLocale()),
      utcMarch2010CalendarPage,
    );
  });
});

it("previous N dates", () => {
  const prepended = previousNDates(
    getDateInTimezone("1995-03-01", utc),
    5,
    utc,
  );
