...
describe("shouldFetch", () => {
  ...
  it("should return true if searchText in state is different", () => {
    ...
    expect(shouldFetchData(props, props, stateWithChangedSearchText, state)).to
      .be.true;
  });

  it("should return true if effective filter in essence in props is different", () => {
    const essenceWithNewFilterClause = wikiTotals.changeFilter(
      wikiTotals.filter.addClause(boolean("isRobot", [true])),
    );
    const propsWithChangedEssence = {
      ...props,
      essence: essenceWithNewFilterClause,
    };
    expect(shouldFetchData(propsWithChangedEssence, props, state, state)).to.be
      .true;
  });
  ...
  it("should return true if sortOn in props is different", () => {
    ...
    expect(shouldFetchData(propsWithChangedSortOn, props, state, state)).to.be
      .true;
  });
  ...
  it("should return true if dimension in props is different", () => {
    ...
    expect(shouldFetchData(propsWithChangedDimension, props, state, state)).to
      .be.true;
  });
});
