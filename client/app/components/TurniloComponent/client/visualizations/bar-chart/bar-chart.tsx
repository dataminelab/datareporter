...
export class BarChart extends BaseVisualization<BarChartState> {
  ...
  renderHoverBubble(hoverInfo: BubbleInfo): JSX.Element {
    ...
    return <SegmentBubble top={topOffset} left={leftOffset} title={segmentLabel} content={measureContent} />;
  }
      ...
      data.forEach((d, i) => {
        ...
        labels.push(
          <div
            ...
            key={segmentValueStr}
            style={{
              right: xAxisStage.width - (coordinate.x + coordinate.stepWidth / 2),
              color: colorTextBar,
            }}>
            {segmentValueStr}
          </div>
  ...
  getYAxisStuff(
    ...
    const axisStage = yAxisStage.changeY(
      yAxisStage.y + (chartStage.height + CHART_TOP_PADDING + CHART_BOTTOM_PADDING) * chartIndex
    );
  ...
  renderChart(
    ...
    const measureLabel = (
      <VisMeasureLabel series={series} datum={dataset.data[0]} showPrevious={essence.hasComparison()} />
    );
    ...
    if (!this.hasValidYExtent(series, mySplitDataset.data)) {
      return {
        chart: (
          <div className="measure-bar-chart" key={series.reactKey()} style={{ width: chartStage.width }}>
            <svg
  ...
  getXValues(maxNumberOfLeaves: number[]): {
    ...
    if (minStepWidth * numPrimarySteps < maxAvailableWidth) {
      stepWidth = Math.max(
        Math.min(maxAvailableWidth / numPrimarySteps, MAX_STEP_WIDTH * essence.splits.length()),
        MIN_STEP_WIDTH
      );
    } else {
  ...
  renderRightGutter(seriesCount: number, yAxisStage: Stage, yAxes: JSX.Element[]): JSX.Element {
    const yAxesStage = yAxisStage.changeHeight(
      (yAxisStage.height + CHART_TOP_PADDING + CHART_BOTTOM_PADDING) * seriesCount
    );

    return (
      <svg style={yAxesStage.getWidthHeight()} viewBox={yAxesStage.getViewBox()}>
        {yAxes}
      </svg>
    );
  }
  ...
  renderSelectionContainer(selectionHighlight: JSX.Element, chartIndex: number, chartStage: Stage): JSX.Element {
    return <div className="selection-highlight-container">{selectionHighlight}</div>;
  }
  ...
  renderInternals(dataset: Dataset) {
    ...
    return (
      <div className="internals measure-bar-charts" style={{ maxHeight: stage.height }}>
        <Scroller
