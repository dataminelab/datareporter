...
describe("Granularity", () => {
  ...
  it("getGranularities appropriately for time", () => {
    ...
    expect(defaults.every((g, i) => granularityEquals(g, expectedDefaults[i]), "time defaults are returned")).to.be
      .true;

    const coarse = getGranularities("time", null, true);
    const expectedCoarseDefaults = ["PT1M", "PT5M", "PT1H", "PT6H", "PT12H", "P1D", "P1W", "P1M"].map(
      granularityFromJS
    );

    expect(coarse.every((g, i) => granularityEquals(g, expectedCoarseDefaults[i]), "coarse time defaults are returned"))
      .to.be.true;
    ...
    expect(
      bucketedBy.every((g, i) => granularityEquals(g, expectedDefaults[i]), "bucketed by time defaults are returned")
    ).to.be.true;
  });
  ...
  it("getGranularities appropriately for number", () => {
    ...
    expect(defaults.every((g, i) => granularityEquals(g, expectedDefaults[i]), "number defaults are returned")).to.be
      .true;
    ...
    expect(
      bucketedBy.every((g, i) => granularityEquals(g, expectedGrans[i]), "bucketed by returns larger granularities")
    ).to.be.true;
  });
  ...
  it("getDefaultGranularityForKind appropriately for number", () => {
    ...
    expect(granularityEquals(customGrans, expected), "default will bucket according to provided customs").to.equal(
      true
    );
  });
