...
describe("Granularity", () => {
  ...
  it("equals appropriately", () => {
    ...
    expect(granularityEquals(numberBucketAction1, numberBucketAction2)).to.be
      .true;
    expect(granularityEquals(numberBucketAction2, numberBucketAction3)).to.be
      .false;
  });

  it("getGranularities appropriately for time", () => {
    const defaults = getGranularities("time");
    let expectedDefaults = ["PT1M", "PT5M", "PT1H", "P1D", "P1W"].map(
      granularityFromJS,
    );

    expect(
      defaults.every(
        (g, i) => granularityEquals(g, expectedDefaults[i]),
        "time defaults are returned",
      ),
    ).to.be.true;

    const coarse = getGranularities("time", null, true);
    const expectedCoarseDefaults = [
      "PT1M",
      "PT5M",
      "PT1H",
      "PT6H",
    ...
    expect(
      coarse.every(
        (g, i) => granularityEquals(g, expectedCoarseDefaults[i]),
        "coarse time defaults are returned",
      ),
    ).to.be.true;

    const bucketedBy = getGranularities(
      "time",
      granularityFromJS("PT12H"),
      false,
    );
    expectedDefaults = ["PT12H", "P1D", "P1W", "P1M", "P3M"].map(
      granularityFromJS,
    );
    ...
    expect(
      bucketedBy.every(
        (g, i) => granularityEquals(g, expectedDefaults[i]),
        "bucketed by time defaults are returned",
      ),
    ).to.be.true;
  });
  ...
  it("getGranularities appropriately for number", () => {
    ...
    expect(
      defaults.every(
        (g, i) => granularityEquals(g, expectedDefaults[i]),
        "number defaults are returned",
      ),
    ).to.be.true;

    const bucketedBy = getGranularities(
      "number",
      granularityFromJS(100),
      false,
    );
    ...
    expect(
      bucketedBy.every(
        (g, i) => granularityEquals(g, expectedGrans[i]),
        "bucketed by returns larger granularities",
      ),
    ).to.be.true;
  });
  ...
  it("getDefaultGranularityForKind appropriately for number", () => {
    ...
    const bucketedBy = getDefaultGranularityForKind(
      "number",
      granularityFromJS(50),
    );
    expected = granularityFromJS(50);

    expect(
      granularityEquals(bucketedBy, expected),
      "default will bucket by provided bucketedBy amount",
    ).to.equal(true);

    const customGrans = getDefaultGranularityForKind(
      "number",
      null,
      [100, 500, 1000, 5000, 10000].map(granularityFromJS),
    );
    expected = granularityFromJS(1000);

    expect(
      granularityEquals(customGrans, expected),
      "default will bucket according to provided customs",
    ).to.equal(true);
  });
  ...
  it("getDefaultGranularityForKind appropriately for time", () => {
    ...
    const customGrans = getDefaultGranularityForKind(
      "time",
      null,
      ["PT1H", "PT8H", "PT12H", "P1D", "P1W"].map(granularityFromJS),
    );
  ...
  it("getsBestBucketUnit appropriately for time defaults depending on coarse flag", () => {
    ...
    const yearLength = new TimeRange({
      start: new Date("1994-02-24T00:00:00.000Z"),
      end: new Date("1995-02-25T00:00:00.000Z"),
    });
    expect(getBestBucketUnitForRange(yearLength, false).toString()).to.equal(
      week,
    );
    expect(getBestBucketUnitForRange(yearLength, true).toString()).to.equal(
      month,
    );

    const monthLength = new TimeRange({
      start: new Date("1995-02-24T00:00:00.000Z"),
      end: new Date("1995-03-25T00:00:00.000Z"),
    });
    expect(getBestBucketUnitForRange(monthLength, false).toString()).to.equal(
      day,
    );
    expect(getBestBucketUnitForRange(monthLength, true).toString()).to.equal(
      week,
    );

    const sevenDaysLength = new TimeRange({
      start: new Date("1995-02-20T00:00:00.000Z"),
      end: new Date("1995-02-28T00:00:00.000Z"),
    });
    expect(
      getBestBucketUnitForRange(sevenDaysLength, false).toString(),
    ).to.equal(oneHour);
    expect(
      getBestBucketUnitForRange(sevenDaysLength, true).toString(),
    ).to.equal(day);

    const threeDaysLength = new TimeRange({
      start: new Date("1995-02-20T00:00:00.000Z"),
      end: new Date("1995-02-24T00:00:00.000Z"),
    });
    expect(
      getBestBucketUnitForRange(sevenDaysLength, false).toString(),
    ).to.equal(oneHour);
    expect(
      getBestBucketUnitForRange(threeDaysLength, true).toString(),
    ).to.equal(twelveHours);

    const dayLength = new TimeRange({
      start: new Date("1995-02-24T00:00:00.000Z"),
      end: new Date("1995-02-25T00:00:00.000Z"),
    });
    expect(getBestBucketUnitForRange(dayLength, false).toString()).to.equal(
      oneHour,
    );
    expect(getBestBucketUnitForRange(dayLength, true).toString()).to.equal(
      sixHours,
    );

    const fourHours = new TimeRange({
      start: new Date("1995-02-24T00:00:00.000Z"),
      end: new Date("1995-02-24T04:00:00.000Z"),
    });
    expect(getBestBucketUnitForRange(fourHours, false).toString()).to.equal(
      fiveMinutes,
    );
    expect(getBestBucketUnitForRange(fourHours, true).toString()).to.equal(
      oneHour,
    );

    const fortyFiveMin = new TimeRange({
      start: new Date("1995-02-24T00:00:00.000Z"),
      end: new Date("1995-02-24T00:45:00.000Z"),
    });
    expect(getBestBucketUnitForRange(fortyFiveMin, false).toString()).to.equal(
      oneMinute,
    );
    expect(getBestBucketUnitForRange(fortyFiveMin, true).toString()).to.equal(
      fiveMinutes,
    );
  });
  ...
  it("getsBestBucketUnit appropriately for time with bucketing and custom granularities", () => {
    ...
    const customs = ["PT1H", "PT8H", "PT12H", "P1D", "P1W"].map(
      granularityFromJS,
    );
    expect(
      getBestBucketUnitForRange(dayLength, false, null, customs).toString(),
    ).to.equal(oneHour);
    ...
    expect(
      getBestBucketUnitForRange(fortyFiveMin, false, null, customs).toString(),
    ).to.equal(oneHour);
  });
  ...
  it("getsBestBucketUnit appropriately for number defaults with bucketing and custom granularities", () => {
    ...
    expect(
      getBestBucketUnitForRange(hundred, false, granularityFromJS(50)),
    ).to.equal(50);
