...
describe("MeasureGroup", () => {
  it("should convert to / from JS", () => {
    const measureGroup = MeasureGroup.fromJS(
      MeasureGroupFixtures.wikiAddedJS(),
    );

    expect(measureGroup.toJS()).to.deep.equal(
      MeasureGroupFixtures.wikiAddedJS(),
    );
  });
  ...
  it("should infer title from name", () => {
    ...
    expect(measureGroup.toJS()).to.deep.equal(
      MeasureGroupFixtures.withTitleInferredJS(),
    );
  });
  ...
  it("should throw when no name given", () => {
    const measureGroupConversion = () =>
      MeasureGroup.fromJS(MeasureGroupFixtures.noNameJS());
  ...
  it("should throw when no measures given", () => {
    ...
    const measureGroupConversion = () =>
      MeasureGroup.fromJS(groupWithNoMeasures);

    expect(measureGroupConversion).to.throw(
      `measure group '${groupWithNoMeasures.name}' has no measures`,
    );
  });
  ...
  it("should throw when empty measures given", () => {
    ...
    const measureGroupConversion = () =>
      MeasureGroup.fromJS(groupWithEmptyMeasures);

    expect(measureGroupConversion).to.throw(
      `measure group '${groupWithEmptyMeasures.name}' has no measures`,
    );
  });
});
