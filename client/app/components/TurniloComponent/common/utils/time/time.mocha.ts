...
describe("Time", () => {
  it("calculates date equality properly", () => {
    ...
    expect(
      datesEqual(
        new Date("1995-02-24T00:00:00.000Z"),
        new Date("1995-02-24T00:02:00.000Z"),
      ),
      "not equal dates",
    ).to.equal(false);
  });
  ...
  it("get walltime day returns day according to walltime", () => {
    ...
    expect(getDayInMonth(date, TZ_Kiritimati), "kiritimati walltime").to.equal(
      9,
    );
  });
  ...
  it("get walltime month returns full month and year according to walltime", () => {
    ...
    expect(formatYearMonth(date, TZ_TIJUANA), "basic tijuana").to.equal(
      "February 1965",
    );
    expect(formatYearMonth(date, TZ_KATHMANDU), "basic kathmandu").to.equal(
      "February 1965",
    );
    expect(formatYearMonth(date, TZ_Kiritimati), "basic kiritimati").to.equal(
      "February 1965",
    );
  });
  ...
  it("get walltime month returns full month and year according to walltime on year boundaries", () => {
    ...
    expect(formatYearMonth(date, TZ_TIJUANA), "y2k tijuana").to.equal(
      "December 1999",
    );
    expect(formatYearMonth(date, TZ_KATHMANDU), "y2k kathmandu").to.equal(
      "January 2000",
    );
    expect(formatYearMonth(date, TZ_Kiritimati), "y2k kiritimati").to.equal(
      "January 2000",
    );
  });
  ...
  describe("scaleTicksFormatter", () => {
    ...
    it("should hide year when just year is the same in all ticks", () => {
      const scale = createScale(
        tz("2019-01-02 12:34", "UTC").toDate(),
        tz("2019-02-01 10:00", "UTC").toDate(),
      );
    ...
    it("should hide hour when just hour is the same in all ticks", () => {
      const scale = createScale(
        tz("2018-12-01 18:00", "UTC").toDate(),
        tz("2019-01-10 18:00", "UTC").toDate(),
      );
    ...
    it("should show full date when just date is the same in all ticks (degenerate case)", () => {
      const scale = createScale(
        tz("2018-01-01", "UTC").toDate(),
        tz("2019-01-01 10:00", "UTC").toDate(),
      );
    ...
    it("should show just hour when only hour is different in some ticks", () => {
      const scale = createScale(
        tz("2019-01-01 12:34", "UTC").toDate(),
        tz("2019-01-01 15:00", "UTC").toDate(),
      );
    ...
    it("should show just date when only date is different in some ticks", () => {
      const scale = createScale(
        tz("2019-01-01 10:00", "UTC").toDate(),
        tz("2019-01-10 10:00", "UTC").toDate(),
      );
    ...
    it("should show full date and hour when everything is different in some ticks", () => {
      const scale = createScale(
        new Date("2018-12-31T23:00"),
        new Date("2019-01-01:T01:00"),
      );
