...
function WidgetDropdownButton({ report, extraOptions, showDeleteOption, onDelete }) {
  const WidgetMenu = (
    <Menu data-test="WidgetDropdownButtonMenu">
      ...
      {showDeleteOption && <Menu.Item onClick={()=> window.location.href=`/reports/${report.id}/source`}>Edit</Menu.Item>}
    ...
    return (
      <div className="widget-wrapper">
        <div className={cx("tile body-container", className)} {...tileProps}>
          ...
          <div className="body-row widget-header">{header || config.name}</div>
          {children}
