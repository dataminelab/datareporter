...
function writePrePrompt(slug) {
  ...
  const datasetHeaders = Array.from(document.querySelectorAll(".widget-header")).filter(
    (header) => header.innerText && header.innerText.trim() !== ""
  );
  let prompt = "Datasets:\n";
...
function DashboardControl({ dashboardConfiguration, headerExtra }) {
  ...
  const handlePromptSend = async () => {
    ...
    const aiProvider = getAIProvider(selectedModel, dashboard.id);
    answer = await aiProvider.getAnswer(promptValue, conversation, prePrompt);
              ...
              <Modal
                ...
                {conversationHistory.length > 0 && (
                  <div style={{ marginTop: 16, textAlign: "right" }}>
                    <Button size="small" onClick={handleClearConversation} type="warning" danger>
