...
export default function useReportModels(report) {
  ...
  const models = useMemo(
    () =>
      filter(allModels, ds => !ds.view_only || ds.id === report.data_source_id),
    [allModels, report.data_source_id],
  );
  const model = useMemo(
    () =>
      find(models, ds => toString(ds.id) === toString(report.data_source_id)) ||
      null,
