...
describe("Embedded Queries", () => {
  ...
  it("is unavailable when public urls feature is disabled", () => {
    cy.createQuery({ query: "select name from users order by name" }).then(
      query => {
        cy.visit(`/queries/${query.id}/source`);
        cy.getByTestId("ExecuteButton").click();
        cy.getByTestId("QueryPageVisualizationTabs", { timeout: 10000 }).should(
          "exist",
        );
        cy.clickThrough(`
      ...
      query => {
        ...
        cy.getByTestId("EmbedIframe")
          ...
          .then(embedUrl => {
            cy.logout();
            cy.visit(embedUrl);
            cy.getByTestId("VisualizationEmbed", { timeout: 10000 }).should(
              "exist",
            );
  ...
  it("can be shared with safe parameters", () => {
    ...
    cy.getByTestId("QueryEditor")
      .get(".ace_text-input")
      .type("SELECT name, slug FROM organizations WHERE id='{{}{{}id}}'{esc}", {
        force: true,
      });
  ...
  it("cannot be shared with unsafe parameters", () => {
    ...
    cy.getByTestId("QueryEditor")
      .get(".ace_text-input")
      .type(
        "SELECT name, slug FROM organizations WHERE name='{{}{{}name}}'{esc}",
        { force: true },
      );
