...
export function createChartThroughUI(
  chartName,
...
export function assertTabbedEditor(
...
export function createDashboardWithCharts(
  title,
  chartGetters,
  widgetsAssertionFn = () => {},
) {
  cy.createDashboard(title).then(dashboard => {
    ...
    const widgetGetters = chartGetters.map(
      chartGetter => `${chartGetter}Widget`,
    );
