...
_operand_.**performAction**(action: Expression)
...
_operand_.**lessThanOrEqual**(ex: any)
...
_operand_.**greaterThanOrEqual**(ex: any)
...
_operand_.**contains**(ex: any, compare?: string)
...
_operand_.**substr**(position: number, length: number)
...
_operand_.**transformCase**(transformType: 'upperCase' | 'lowerCase')
...
_operand_.**indexOf**(substr: string)
...
_operand_.**customTransform**(custom: string, outputType?: 'NULL', 'BOOLEAN', 'NUMBER', 'TIME', 'STRING')
...
_operand_.**numberBucket**(size: number, offset: number = 0)
...
_operand_.**timeBucket**(duration: any, timezone?: string)
...
_operand_.**timeFloor**(duration: any, timezone?: string)
...
_operand_.**timeShift**(duration: any, step: number, timezone?: string)
...
_operand_.**timeRange**(duration: any, step: number, timezone?: string)

Constructs a range with the operand time as one end point and the other endpoint determined by shifting the provided time by `duration` _ `step` within the given `timezone`.
Creates a TimeRange of size `duration` _ `step`.
...
_operand_.**timePart**(part: string, timezone?: string)
...
_operand_.**cast**(castType: string)
...
_operand_.**split**(splits: any, name?: string, dataName?: string)
...
Dataset.fromJS([
  ...
  {
    Cut: 'Great',
    data: [{ cut: 'Great', price: 124, time: null }],
  },
  {
    Cut: 'Wow',
    data: [
...
_operand_.**apply**(name: string, ex: any)
...
_operand_.**sort**(ex: any, direction: string)
...
_operand_.**select**(...attributes: string[])
...
_operand_.**countDistinct**(ex: any)
...
_operand_.**quantile**(ex: any, quantile: number)
...
_operand_.**customAggregate**(custom: string)
...
_operand_.**sqlAggregate**(sql: string)
