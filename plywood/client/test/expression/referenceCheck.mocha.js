...
describe("reference check", () => {
  ...
  describe("errors", () => {
    ...
    it("fails when discovering that the types mismatch via split", () => {
      const ex = ply()
        ...
        .apply(
          "Cuts",
          $("diamonds").split("$cut", "Cut").apply("TotalPrice", "$Cut * 10"),
        );
  ...
  describe("resolves in type context", () => {
    ...
    it("works in a simple reference case", () => {
      const ex1 = $("x");
      const ex2 = $("x", "NUMBER");
      expect(ex1.changeInTypeContext(typeContext).toJS()).to.deep.equal(
        ex2.toJS(),
      );
    });
  ...
  describe("resolves in context", () => {
    ...
    it("works in a basic case", () => {
      ...
      const ex2 = ply()
        ...
        .apply(
          "subData",
          ply().apply("x", "$^num:NUMBER + 1").apply("y", "$x:NUMBER + 2"),
        );
    ...
    it("works from context 1", () => {
      ...
      const ex2 = $("diamonds", "DATASET").apply(
        "pricePlus2",
        "$price:NUMBER + 2",
      );
    ...
    it("works from context 2", () => {
      ...
      const ex2 = ply()
        ...
        .apply(
          "countPlusSeventy",
          "$Diamonds:DATASET.count() + $^seventy:NUMBER",
        );
    ...
    it("works with countDistinct", () => {
      ...
      const ex2 = ply()
        .apply(
          "DistinctColors",
          "$^diamonds:DATASET.countDistinct($color:STRING)",
        )
    ...
    it("a total", () => {
      ...
      const ex2 = ply()
        .apply(
          "diamonds",
          $("diamonds", 1, "DATASET").filter($("color", "STRING").is("D")),
        )
      ...
      const ex2 = ply()
        ...
        .apply(
          ...
          $("diamonds", "DATASET")
            ...
            .apply(
              "PercentOfTotal",
              "$diamonds:DATASET.sum($price:NUMBER) / $^TotalPrice:NUMBER",
            )
            .sort("$Count:NUMBER", "descending")
            .limit(2)
            .apply(
              "Carats",
              $("diamonds", "DATASET")
                .split($("carat", "NUMBER").numberBucket(0.25), "Carat")
                .apply("Count", $("diamonds", "DATASET").count())
                .sort("$Count:NUMBER", "descending")
                .limit(3),
            ),
    ...
    it("works with join", () => {
      const ex1 = ply()
        ...
        .apply(
          "Data2",
          $("diamonds").filter($("price").overlap(105, 305).not()),
        )
        .apply(
          "Cuts",
          $("Data1")
      ...
      const ex2 = ply()
        .apply(
          ...
          $("diamonds", 1, "DATASET").filter(
            $("price", "NUMBER").overlap(105, 305),
          ),
        )
        .apply(
          "Data2",
          $("diamonds", 1, "DATASET").filter(
            $("price", "NUMBER").overlap(105, 305).not(),
          ),
        )
        .apply(
          "Cuts",
          $("Data1", "DATASET")
            .split("$cut:STRING", "Cut", "K1")
            .join($("Data2", "DATASET").split("$cut:STRING", "Cut", "K2"))
