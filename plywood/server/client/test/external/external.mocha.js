...
const context = {
  ...
  wikiCmp: wikiDataset.addExpression(
    Expression._.filter(
      $('time').overlap(
        TimeRange.fromJS({
...
describe('External', () => {
  ...
  describe('constructor', () => {
    it('type checks', () => {
      expect(wikiDataset.derivedAttributes['language'].toString()).to.equals(
        '$language:STRING.substr(0,100)',
      );
      expect(wikiDataset.derivedAttributes['pageTm'].toString()).to.equals(
        '$page:STRING.concat("(TM)")',
      );
    });
  });
  ...
  describe('#addExpression / #getRaw', () => {
    ...
    it('runs through a life cycle', () => {
      ...
      expect(filteredValueExternal.valueExpression.toString()).to.equal(
