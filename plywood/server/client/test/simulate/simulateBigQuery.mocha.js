...
describe('simulate BigQuery', () => {
  it('works in advanced case', () => {
    ...
    expect(queryPlan[2]).to.deep.equal([
      sane`
      SELECT
      FORMAT_DATETIME('%Y-%m-%d 00:00:00Z', CAST(\`time\` AS DATETIME)) AS \`Timestamp\`,
      SUM(\`price\`) AS \`TotalPrice\`
      FROM \`diamonds\` AS t
    ...
    expect(queryPlan[3]).to.deep.equal([
      sane`
      SELECT
      FLOOR(\`carat\` / 0.25) * 0.25 AS \`Carat\`,
      COALESCE(COUNT(*), 0) AS \`Count\`
      FROM \`diamonds\` AS t
      WHERE (((\`color\`='D') AND (\`cut\`='some_cut')) AND (TIMESTAMP('2015-03-13T07:00:00.000Z')<=\`time\` AND \`time\`<TIMESTAMP('2015-03-14T07:00:00.000Z')))
      GROUP BY 1
      ORDER BY \`Count\` DESC
      LIMIT 3
    `,
