...
export abstract class AttributeParser {
  ...
  protected abstract _parseAttributes(
    attributes: InputAttribute[],
  ): OutputAttribute[];

  //Marks  unsupported data types
  protected markUnsupportedAttributes(
  ...
  protected fitNewAttributes(
    newAttributes: OutputAttribute[],
  ): OutputAttribute[] {
    ...
    for (const attribute of this.attributes) {
      ...
      if (newIndex === -1)
        result.push({ ...attribute, nativeType: attribute.type });
