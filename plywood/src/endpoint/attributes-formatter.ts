...
const validation = (req: Request) => {
  ...
  if (!Array.isArray(attributes))
    throw new FieldError("Should be an array", { fieldName: "attributes" });

  for (const [i, attribute] of attributes.entries()) {
    const name = attribute.name;
    const type = attribute.type;
    if (name === undefined)
      throw new FieldError(`Name is missing in attributes at index ${i}`, {
        fieldName: "attributes.name",
      });
    if (type === undefined)
      throw new FieldError(`Type is missing in attributes at index ${i}`, {
...
export const attributesFormatterEndpoint = (req: Request, res: Response) => {
  ...
  // parsing
  const body = req.body as Input;
  const AttributeParserInstance = AttributeParserFactory.getAttributeParser(
    body.engine,
  );
