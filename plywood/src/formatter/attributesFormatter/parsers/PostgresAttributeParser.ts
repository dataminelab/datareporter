import { PostgresExternal, PostgresSQLDescribeRow } from "reporter-plywood";
...
export class PostgresAttributeParser extends AttributeParser {
  ...
  protected _parseAttributes(attributes: InputAttribute[]): OutputAttribute[] {
    const columns: PostgresSQLDescribeRow[] = attributes.map(attribute => {
      const result: PostgresSQLDescribeRow = {
        name: attribute.name,
        sqlType: attribute.type,
      };
    ...
    const newAttributes = PostgresExternal.postProcessIntrospect(columns).map(
      atr => {
        if (!atr) return;
        return {
