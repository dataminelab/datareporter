import {
  DataCube,
  DataCubeJS,
} from "../turnillo/common/models/data-cube/data-cube";
...
import {
  ...
  Split,
  bucketToAction,
} from "../turnillo/common/models/split/split";
import {
  TimeShiftEnv,
  TimeShiftEnvType,
} from "../turnillo/common/models/time-shift/time-shift-env";
...
export const hashToExpression = (
  ...
  const res: Expression =
    splits.length() > 0
      ? queryWithMeasures.apply(
          SPLIT,
          applySplit(0, essence, timeShiftEnv, $main),
        )
      : queryWithMeasures;
...
function applySubSplit(
  ...
  return (query: Expression) => {
    ...
    return query.apply(
      SPLIT,
      applySplit(nestingLevel, essence, timeShiftEnv, $main),
    );
  };
}
