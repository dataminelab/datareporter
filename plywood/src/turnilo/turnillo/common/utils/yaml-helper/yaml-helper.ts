...
import { AttributeInfo } from "reporter-plywood";
import {
  CLUSTER,
  CUSTOMIZATION,
  DATA_CUBE,
...
function clusterToYAML(cluster: Cluster, withComments: boolean): string[] {
  let lines: string[] = [`name: ${cluster.name}`];
...
function attributeToYAML(attribute: AttributeInfo): string[] {
  let lines: string[] = [`name: ${attribute.name}`, `type: ${attribute.type}`];
...
function measureToYAML(measure: Measure): string[] {
  let lines: string[] = [`name: ${measure.name}`, `title: ${measure.title}`];
