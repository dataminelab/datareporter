...
class ReportFavoriteResource(BaseResource):
    def post(self, report_id: int):
        ...
        fav = models.Favorite(org_id=self.current_org.id, object=report, user=self.current_user)
        models.db.session.add(fav)
        ...
        self.record_event({"action": "favorite", "object_id": report.id, "object_type": "report"})
    ...
    def delete(self, report_id: int):
        ...
        models.db.session.commit()
        self.record_event({"action": "unfavorite", "object_id": report.id, "object_type": "report"})
