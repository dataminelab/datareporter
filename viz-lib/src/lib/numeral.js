  ...
  numeral._ = _ = {
    ...
    stringToNumber: function (string) {
      ...
      if (options.zeroFormat && string === options.zeroFormat) {
        ...
        value *=
          (string.split("-").length + Math.min(string.split("(").length - 1, string.split(")").length - 1)) % 2
            ? _.Big(1)
            : _.Big(-1);
      ...
      if (_valArray.length > 2) {
        ...
        if (_valArray.length < 2) {
          ...
          if (_valArray[0].length === 1) {
            return (
              !!_valArray[0].match(/^\d+$/) && !_valArray[0].match(_thousandRegEx) && !!_valArray[1].match(/^\d+$/)
            );
          } else {
            return (
              ...
              output =
                i === symbols.after.length - 1
                  ...
                  : numeral._.insert(
                      output,
                      " ",
                      -(symbols.after.length - (1 + i) + locale.currency.symbol.length - 1)
                    );
              break;
          }
        }
