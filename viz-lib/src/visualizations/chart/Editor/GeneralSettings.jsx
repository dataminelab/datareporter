    ...
    <React.Fragment>
      ...
      {map(mappedColumns, (value, type) => (
        <ColumnMappingSelect
          ...
          // @ts-expect-error ts-migrate(2322) FIXME: Type '(column, type) => void' is not ass... Remove this comment to see the full error message
      ...
      {includes(["pie"], options.globalSeriesType) && (
        ...
        <Section>
          ...
          <Select label="Sort" defaultValue={options.piesort} onChange={(val) => onOptionsChange({ piesort: val })}>
            {/* @ts-expect-error ts-migrate(2339) FIXME: Property 'Option' does not exist on type '({ class... Remove this comment to see the full error message */}
      ...
      <Section>
        <Input
          ...
          onChange={(e) => debouncedOnOptionsChange({ linkFormat: e.target.value })}
          disabled={!(options.enableLink === true)}
        />
      </Section>
    </React.Fragment>
